    ...
    if (runmod) {
        ...
        if (! xvalid) {
            ...
            for (jj in 1:nfold) {
                ...
                incvec <- dftemp$pn <= 0.1

                ip1 <- (1:nrow(dftemp))[!incvec]
                ip2 <- (1:nrow(dftemp))[incvec]
                ...
                datstan <- list(n = nrow(dftemp),
                                ...
                                tss = log(dftemp$vss),
                                ns = c(length(ip1), length(ip2)),
                                tn1 = log(dftemp$tn-dftemp$dtn)[ip1],
                                tn2 = (dftemp$tn-dftemp$dtn)[ip2])
