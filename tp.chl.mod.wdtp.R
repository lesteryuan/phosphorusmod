...
ntumodel <- function(df1, varout = NULL, runmod = T) {
    ...
    modstan <- '
        data {
            ...
            real a[2];             // coefficients for the Chl-DTP relationship
    ...
    if (runmod) {
        ...
        fit <- stan(model_code = modstan,
                    data = datstan, iter = 500, chains = nchains,
                    warmup = 300, thin = 2)
        return(fit)
    }
...
#ntumodel(dat.merge.all, varout.p, runmod = F)
